...
#endif
{
	sawParameter = new juce::AudioParameterFloat(juce::ParameterID{"Parameter 1", 1}, "Saw",  -60, 6, -6);
	addParameter(sawParameter);
	sineParameter = new juce::AudioParameterFloat(juce::ParameterID{"Parameter 2", 1}, "Sine", -60, 6, -6);
	addParameter(sineParameter);
	squareParameter = new juce::AudioParameterFloat(juce::ParameterID{"Parameter 3", 1}, "Square", -60, 6, -6);
	addParameter(squareParameter);
	triangleParameter = new juce::AudioParameterFloat(juce::ParameterID{"Parameter 4", 1}, "triangle", -60, 6, -6);
	addParameter(triangleParameter);
	lowPassParameter = new juce::AudioParameterFloat(juce::ParameterID{"Parameter 5", 1}, "Low pass", 20, 20000, 20000);
	addParameter(lowPassParameter);
	highPassParameter = new juce::AudioParameterFloat(juce::ParameterID{"Parameter 6", 1}, "High pass", 20, 20000, 20);
	addParameter(highPassParameter);
	
	gainParameter = new juce::AudioParameterFloat(juce::ParameterID{"Parameter 7", 1}, "Gain", -24.0f, 12.0f, 0.0f);
	addParameter(gainParameter);
	
	distortParameter = new juce::AudioParameterFloat(juce::ParameterID{"Parameter 8", 1}, "Distortion", -24.0f, 12.0f, 0.0f);
	addParameter(distortParameter);
}
...
SynthDemov2AudioProcessor::~SynthDemov2AudioProcessor()
{
	...
	if(distortParameter != nullptr)
		delete distortParameter;
}
...
void SynthDemov2AudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
	...
	lowPassFilter.prepare(lowPassProcessSpec);
	...
	highPassFilter.prepare(highPassProcessSpec);
...
void SynthDemov2AudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
	...
	synth.setSawVolume(*sawParameter);
	synth.setSineVolume(*sineParameter);
	synth.setSquareVolume(*squareParameter);
	synth.settriangleVolume(*triangleParameter);
	synth.render(buffer, midiMessages);
	
	lowPassFilter.coefficients = juce::dsp::IIR::Coefficients<float>::makeLowPass(spec.sampleRate, *lowPassParameter);
	lowPassFilterR.coefficients = juce::dsp::IIR::Coefficients<float>::makeLowPass(spec.sampleRate, *lowPassParameter);
	
	highPassFilter.coefficients = juce::dsp::IIR::Coefficients<float>::makeHighPass(spec.sampleRate, *highPassParameter);
	highPassFilterR.coefficients = juce::dsp::IIR::Coefficients<float>::makeHighPass(spec.sampleRate, *highPassParameter);
	
	float gain = Adin::Conversions::DBtoLinear(*gainParameter);
	
	float distort = Adin::Conversions::DBtoLinear(*distortParameter);

	for(int i=0; i<buffer.getNumChannels(); i++) {
		for(int j=0; j<buffer.getNumSamples(); j++) {
			float sample = buffer.getSample(i, j);
			sample = atan(sample * distort);
			if(i==0) buffer.setSample(i, j, lowPassFilter.processSample(sample));
			else buffer.setSample(i, j, lowPassFilterR.processSample(sample));
			sample = buffer.getSample(i, j);
			if(i==0) buffer.setSample(i, j, highPassFilter.processSample(sample));
